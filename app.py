...
# --- نوار سمت راست: تنظیمات سقف آف و مرخصی ---
st.sidebar.header("⚙️ تنظیمات سقف آف (OFF) و مرخصی")

max_off_midweek = st.sidebar.number_input("حداکثر آف/مرخصی (شنبه تا چهارشنبه):", min_value=1, max_value=15, value=4)
max_off_thursday = st.sidebar.number_input("حداکثر آف/مرخصی (پنج‌شنبه):", min_value=1, max_value=15, value=6)
max_off_friday = st.sidebar.number_input("حداکثر آف/مرخصی (جمعه):", min_value=1, max_value=15, value=8)

st.sidebar.markdown("---")
st.sidebar.header("📋 ثبت یکجای کامنت/درخواست‌ها")

st.sidebar.caption("💡 نمونه فرمت ورود اطلاعات:")
st.sidebar.text("Maryam Khojastehpoor: 5 شهریور مرخصی\nRazieh: 10 شهریور صبح\nZareei: ۱۵ شهریور عصر")

bulk_text = st.sidebar.text_area(
    "کامنت‌ها و درخواست‌ها را یک‌جا وارد کنید:", 
    height=150,
    placeholder="نام کارشناس: روز نوع_درخواست\nمثال:\nRazieh Ansari: 10 شهریور مرخصی\nElahe: ۱۵ شهریور 14-22"
)
...
if st.sidebar.button("📥 پردازش و ثبت کلی درخواست‌ها"):
    ...
    # نقشه تبدیل و تشخیص هوشمند شیفت‌ها و مرخصی
    shift_patterns = {
        "مرخصی": ["مرخصی", "vacation", "leave"],
        "OFF": ["off", "آف", "تعطیل"],
    ...
    for line in lines:
        ...
        # ۲. تبدیل اعداد فارسی و استخراج عدد روز (۱ تا ۳۱)
        ...
        if not detected_shift:
            detected_shift = "مرخصی"  # حالت پیش‌فرض برای درخواست‌ها
...
with tab2:
    ...
    if st.session_state.requests_list:
        ...
        st.info("هنوز هیچ درخواستی ثبت نشده است.")
...
if solve_button or st.session_state.requests_list:
    ...
    # اعمال دقیق درخواست‌های ثبت‌شده (جایگزینی عنوان مرخصی یا شیفت درخواست‌شده)
    ...
    with tab1:
        st.success(f"✅ شیفت‌بندی شهریور ۱۴۰۵ با احتساب تمامی درخواست‌های مرخصی و سقف‌های تعیین‌شده محاسبه شد.")
